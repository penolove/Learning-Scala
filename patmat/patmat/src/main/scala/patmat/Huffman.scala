...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
        def scanchar(char: Char, ret:List[(Char, Int)]): List[(Char, Int)]={
          if(ret.isEmpty){
            (char,1)::ret
          }else ret.head match{
            case (ch,num) =>
              if(ch==char)(char,num+1)::ret.tail
              else ret.head :: scanchar(char,ret.tail)
          }
        }
    		if( chars.isEmpty )
    			List[(Char, Int)]()
    		else
    			scanchar(chars.head, times(chars.tail))
    }
   ...
   */
   def insertTree(freq:(Char,Int),Lret:List[Leaf]):List[Leaf] =Lret match{
        case List() => List(Leaf(freq._1,freq._2))
        case y::ys => if(freq._2<weight(y)) Leaf(freq._1,freq._2)::Lret else y::insertTree(freq,ys)
   }
   
   def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
      case List()=>List()
      case y::ys => insertTree((y._1,y._2),makeOrderedLeafList(ys))
   }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def redec(subtree :CodeTree,bitts:List[Bit]):List[Char]= subtree match{
        case Leaf(char,_)=>char::redec(tree,bitts)
        case Fork(left,right,_,_)=>{
          if(bitts.isEmpty) List[Char]()
          else bitts.head match{
            case 0 => redec(left,bitts.tail)
            case 1 => redec(right,bitts.tail)
          }
        }
      }
      redec(tree,bits)
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
        def encodeHelp(subtree:CodeTree,text :List[Char]):List[Bit]={
